...
class PyBullet(core.View):
    ...
    def __init__(self, scene: core.Scene, scratch_dir=tempfile.mkdtemp()):
        ...
        self._physics_client = _BulletClient(pb.GUI_SERVER)  # pb.GUI
        ...
        self._physics_client.setRealTimeSimulation(0)  # disable real-time simulation
    ...
    def _add_object(self, obj: core.FileBasedObject) -> Optional[int]:
        ...
        scale = obj.scale[0]
        assert (
            obj.scale[1] == obj.scale[2] == scale
        ), "Pybullet does not support non-uniform scaling"
    ...
    def run(
        self, frame_start: int = 0, frame_end: Optional[int] = None, callback=None
            ...
            for collision in contact_points:
                ...
                if normal_force > 1e-6:
                    collisions.append(
                        {
                            "instances": (
                                self._obj_idx_to_asset(body_b),
                                self._obj_idx_to_asset(body_a),
                            ),
                            "position": position_b,
    ...
    def _obj_idx_to_asset(self, idx):
        assets = [
            asset
            for asset in self.scene.assets
            if asset.linked_objects.get(self) == idx
        ]
        if len(assets) == 1:
            return assets[0]
        elif len(assets) == 0:
            ...
            raise RuntimeError(
                "Multiple assets linked to same pybullet object. That should never happen"
            )
...
def register_physical_object_setters(
    obj: core.PhysicalObject, obj_idx, physics_client: _BulletClient
):
...
def set_position(
    object_idx, position, asset, physics_client: _BulletClient
...
def set_quaternion(
    object_idx, quaternion, asset, physics_client: _BulletClient
):  # pylint: disable=unused-argument
...
def set_velocity(
    object_idx, velocity, asset, physics_client: _BulletClient
):  # pylint: disable=unused-argument
    _, angular_velocity = physics_client.getBaseVelocity(
        object_idx
    )  # reuse existing angular velocity
    physics_client.resetBaseVelocity(object_idx, velocity, angular_velocity)
...
def set_friction(
    object_idx, friction: float, asset, physics_client: _BulletClient
):  # pylint: disable=unused-argument
    if friction < 0:
